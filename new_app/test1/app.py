...
class MainFrame(Frame):
	...
	def addToBillLog(self,data):
	    ...
	    self.state["billData"][str(self.state["billListIndex"]+1)]= str(data["itemName"])+" X "+str(data["quantity"])+" = "+str(data["cost"]*data["quantity"])
	...
	def SendBillToProcess(self):
		...
		newWindow = Toplevel(self.master)
	    	BillTab(newWindow,self.state["billData"],self.state["bill_no"])
		self.ClearBillLog()
	    	newWindow.resizable(True,False)
	    	newWindow.update()
	    	newWindow.geometry(newWindow.geometry())
    	    	newWindow.mainloop()
	...
	def QuitSettingsTab(self):
		...
		self.Print=Label(self,text='  Print  ',background=self.bgcolor,relief=SUNKEN,font=("Helvetica",15))
                self.Print.grid(column=0,columnspan=2,row=24,padx=1,pady=1,sticky=N+W)
		self.Prev=Label(self,text='Prev Slide',background=self.bgcolor,font=self.customFonts)
                self.Prev.grid(column=2,row=24,padx=1,pady=1,sticky=N+W)
		self.Next=Label(self,text='Next Slide',background=self.bgcolor,font=self.customFonts)
                self.Next.grid(column=4,row=24,padx=1,pady=1,sticky=N+E)
